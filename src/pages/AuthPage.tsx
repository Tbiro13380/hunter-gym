...
/** Mensagens do Supabase Auth com dicas em PT-BR */
function formatAuthError(message: string): string {
  const m = message.toLowerCase()
  if (m.includes('rate limit') || m.includes('email rate')) {
    return (
      'Limite de envio de e-mail do Supabase atingido (muitas tentativas ou cota do plano). ' +
      'Soluções: (1) Em Supabase → Authentication → Providers → Email, desative "Confirm email" no projeto de dev para criar conta sem e-mail. ' +
      '(2) Use "Continuar com Google". (3) Aguarde cerca de 1 hora e tente de novo.'
    )
  }
  return message
}
...
export default function AuthPage() {
  ...
  const { login, register, signInWithMagicLink, signInWithGoogle } = useAuthStore()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (tab === 'register') {
        ...
        if (!profileExists) {
          const uid = useAuthStore.getState().session?.accountId
          initProfile(name.trim(), uid)
        }
      } else {
        ...
        if (!profileExists && sess) {
          initProfile(sess.name, sess.accountId)
        }
      }
    } finally {
      setLoading(false)
    ...
    <div className="min-h-[100dvh] bg-[#0a0a0f] flex flex-col items-center justify-center px-5 py-8 relative overflow-hidden">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {tab === 'register' && (
              <p className="text-[10px] text-[#64748b]">
                {SUPABASE_ENABLED
                  ? 'Mínimo 6 caracteres. Conta gerenciada pelo Supabase (Auth).'
          ...
          {SUPABASE_ENABLED && (
            <div className="flex flex-col gap-2 pt-2 border-t border-[#2a2a3a] mt-2">
              <button
                type="button"
                disabled={loading}
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (r.error) setError(formatAuthError(r.error))
                    else if (r.info) setSuccess(r.info)
                  } finally {
                    setLoading(false)
                  }
                }}
                className="w-full py-3 rounded-xl border border-[#2a2a3a] bg-[#1a1a26] text-sm text-white hover:border-[#7c3aed]/40 transition-colors"
              >
              ...
              <button
                ...
                className="w-full py-3 rounded-xl border border-[#2a2a3a] bg-[#1a1a26] text-sm text-white hover:border-[#7c3aed]/40 transition-colors flex items-center justify-center gap-2"
              >
        ...
        {tab === 'login' && !SUPABASE_ENABLED && (
          <ExistingAccounts onSelect={(e) => setEmail(e)} />
        )}
      </div>

      {/* Footer */}
      <p className="text-[#1a1a26] text-[10px] mt-8">
        Hunter Gym v1.0 · {SUPABASE_ENABLED ? 'Supabase + sync na nuvem' : 'Dados armazenados localmente'}
      </p>
    </div>
  )
}
